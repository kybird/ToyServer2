#include "System/Timer/AsioTimer.h"
...
std::shared_ptr<TimerHandle> AsioTimer::SetTimeout(std::chrono::milliseconds delay, std::function<void()> callback) {
    ...
    std::shared_ptr<TimerHandle> handle(rawHandle, [this](TimerHandle *ptr) {
        ptr->Cancel();         // Ensure cleanup
        _handlePool.Push(ptr); // Back to pool
    });
    ...
    timer->async_wait(boost::asio::bind_allocator(boost::asio::recycling_allocator<void>(),
